...
void BLEMotionService::_executeCommand(const String& command) {
    ...
    if (command == "enable") {
        ...
        cfg.effectOnUp = command.substring(5);
        _processor->setConfig(cfg);
        Serial.printf("[MOTION BLE] ✓ Effect map UP set: %s\n", cfg.effectOnUp.c_str());
    } else if (command.startsWith("isdown ") && _processor) {
        MotionProcessor::Config cfg = _processor->getConfig();
        cfg.effectOnDown = command.substring(7);
        _processor->setConfig(cfg);
        Serial.printf("[MOTION BLE] ✓ Effect map DOWN set: %s\n", cfg.effectOnDown.c_str());
    } else if (command.startsWith("isleft ") && _processor) {
        MotionProcessor::Config cfg = _processor->getConfig();
        cfg.effectOnLeft = command.substring(7);
        _processor->setConfig(cfg);
        Serial.printf("[MOTION BLE] ✓ Effect map LEFT set: %s\n", cfg.effectOnLeft.c_str());
    } else if (command.startsWith("isright ") && _processor) {
        MotionProcessor::Config cfg = _processor->getConfig();
        cfg.effectOnRight = command.substring(8);
        _processor->setConfig(cfg);
        Serial.printf("[MOTION BLE] ✓ Effect map RIGHT set: %s\n", cfg.effectOnRight.c_str());
...
void BLEMotionService::ConfigCallbacks::onWrite(BLECharacteristic* pCharacteristic) {
    ...
    // "gestureClashIntensity": 0-255, "effectMapUp": "flicker",
    // "effectMapDown": "...", "effectMapLeft": "...", "effectMapRight": "...",
    // "debugLogs": bool}
    JsonDocument doc;
    ...
    const bool hasMapUp = !doc["effectMapUp"].isNull();
    const bool hasMapDown = !doc["effectMapDown"].isNull();
    const bool hasMapLeft = !doc["effectMapLeft"].isNull();
    const bool hasMapRight = !doc["effectMapRight"].isNull();
    ...
    if (_service->_processor &&
        (hasGesturesEnabled || hasIgnitionIntensity || hasRetractIntensity || hasClashIntensity ||
        ...
        if (hasMapUp) {
            cfg.effectOnUp = (const char*)doc["effectMapUp"];
        }
        if (hasMapDown) {
            cfg.effectOnDown = (const char*)doc["effectMapDown"];
        }
        if (hasMapLeft) {
            cfg.effectOnLeft = (const char*)doc["effectMapLeft"];
        }
        if (hasMapRight) {
            cfg.effectOnRight = (const char*)doc["effectMapRight"];
        }
        if (hasDebugLogs) {
            cfg.debugLogsEnabled = (bool)doc["debugLogs"];
        }
        _service->_processor->setConfig(cfg);
        Serial.printf("[MOTION BLE] Gesture update: enabled=%s ignition=%u retract=%u clash=%u\n",
                      cfg.gesturesEnabled ? "true" : "false",
