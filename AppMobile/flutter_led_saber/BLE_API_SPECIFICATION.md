# LedSaber BLE API - Specifica per App Mobile

Versione: 3.1
Data: 2025-12-20 (aggiornata al firmware)
Target: App Mobile (Flutter/React Native/Kotlin)
...
3. [Caratteristiche gia' implementate (storico)](#-caratteristiche-ble-gia-implementate-storico)
...
## Stato attuale (firmware reale)

Questa specifica e' allineata al firmware presente in:

- include/BLELedController.h, src/BLELedController.cpp
- src/BLECameraService.h, src/BLECameraService.cpp
- src/BLEMotionService.h, src/BLEMotionService.cpp
- include/OTAManager.h, src/OTAManager.cpp

Non ci sono "gap" rispetto al codice attuale: le characteristic richieste per app,
inclusi ignition/retract, effects list e bladeState, sono gia' presenti.
Nota: la sezione "gia' implementate (storico)" e' mantenuta solo come riferimento.
...
|---------------|------|-----|---------|-------------|
| LED State | `beb5483e-36e1-4688-b7f5-ea07361b26a8` | READ, NOTIFY | JSON | Stato completo LED e bladeState |
| LED Color | `d1e5a4c3-eb10-4a3e-8a4c-1234567890ab` | WRITE, WRITE_NR | JSON | Imposta colore RGB `{"r":255,"g":0,"b":0}` |
| LED Effect | `e2f6b5d4-fc21-5b4f-9b5d-2345678901bc` | WRITE, WRITE_NR | JSON | Imposta effetto `{"mode":"pulse","speed":100}` |
| LED Brightness | `f3e7c6e5-0d32-4c5a-ac6e-3456789012cd` | WRITE, WRITE_NR | JSON | Luminosita' + on/off (0-255) `{"brightness":60,"enabled":true}` |
...
| Device Control | `c7f8e0d9-5b87-1a2b-be9d-7890abcdef23` | WRITE | JSON | Comandi ignition/retract/reboot/sleep/boot_config |
| Effects List | `d8f9e1ea-6c98-2b3c-cf0e-890abcdef234` | READ | JSON | Lista effetti e parametri |
...
```json
{
  ...
  "foldPoint": 72,
  "autoIgnitionOnBoot": false,
  "autoIgnitionDelayMs": 5000,
  "motionEnabled": false,
  "gestureClashEffect": "clash",
  "gestureClashDurationMs": 500
}
```
...
La characteristic restituisce una lista JSON con ID, nome, parametri e temi.
Usare questa lista per evitare hardcoding in app.
...
| OTA Status | `d1e5a4c4-eb10-4a3e-8a4c-1234567890ab` | READ, NOTIFY | String | Stato OTA (numero): "STATE:ERROR_MSG" |
| OTA Progress | `f3e7c6e6-0d32-4c5a-ac6e-3456789012cd` | READ, NOTIFY | String | "PERCENT:RECEIVED:TOTAL" |
| OTA Control | `e2f6b5d5-fc21-5b4f-9b5d-2345678901bc` | WRITE | Binary | 0x01=START, 0x02=ABORT, 0x03=VERIFY, 0x04=REBOOT |
| OTA Data | `beb5483f-36e1-4688-b7f5-ea07361b26a8` | WRITE | Binary | Chunk firmware (max 512 bytes) |

#### Protocollo OTA (sintesi)
...
2. START Command (0x01 + size uint32 LE)
...
3. UPLOAD CHUNKS (write without response consigliato)
...
Android: usare `requestMtu(517)` dopo connessione. iOS: MTU automatico.

---

Se il device disconnette durante START (puo' succedere), fare reconnect automatico.
Dimensione chunk consigliata: min(512, mtu - 3) quando possibile.
...
| Characteristic | UUID | Ops | Formato | Descrizione |
|---------------|------|-----|---------|-------------|
| Camera Status | `6eb5483e-36e1-4688-b7f5-ea07361b26a8` | READ, NOTIFY | JSON | Stato camera |
| Camera Control | `7dc5a4c3-eb10-4a3e-8a4c-1234567890ab` | WRITE | String | init/capture/start/stop/reset_metrics |
| Camera Metrics | `8ef6b5d4-fc21-5b4f-9b5d-2345678901bc` | READ | JSON | Metriche camera |
| Camera Flash | `9fe7c6e5-0d32-4c5a-ac6e-3456789012cd` | READ, WRITE | JSON | {"enabled":true,"brightness":255} |

Status JSON:

```json
{"initialized":true,"active":false,"fps":10,"totalFrames":120,"failedCaptures":2}
```

Metrics JSON:

```json
{"totalFramesCaptured":120,"failedCaptures":2,"lastFrameSize":12345,"lastCaptureTime":42,"currentFps":10,"heapFree":123456,"psramTotal":4194304,"psramFree":2097152}
```

---

### Service: Motion (`6fafc401-1fb5-459e-8fcc-c5c9c331914b`)

| Characteristic | UUID | Ops | Formato | Descrizione |
|---------------|------|-----|---------|-------------|
| Motion Status | `7eb5583e-36e1-4688-b7f5-ea07361b26a9` | READ, NOTIFY | JSON | Stato motion + metriche (include centroid e grid) |
...
```json
{"enabled":true,"motionDetected":false,"quality":128,"intensity":12,"direction":"up","speed":1.2,"confidence":80,"gesture":"none","centroidValid":false,"gridRows":6,"gridCols":6,"blockSize":16}
```
...
| Characteristic | UUID | Ops | Formato | Descrizione |
|---------------|------|-----|---------|-------------|
| **Motion Status** | `7eb5583e-36e1-4688-b7f5-ea07361b26a9` | READ, NOTIFY | JSON | Intensità, direzione, velocità, gesture + centroid/grid |
...
| **Motion Events** | `9ef6c5d4-fc21-5b4f-9b5d-2345678901bd` | NOTIFY | JSON | Eventi (shake_detected, motion_started, motion_ended, gesture_detected) |
...
## ✅ Caratteristiche BLE GIA' IMPLEMENTATE (storico)

### **1. DEVICE_CONTROL** (gia' presente nel Service LED)
...
### **2. EFFECTS_LIST** (gia' presente nel Service LED)
...
Nota: il firmware non invia il campo `description`; se serve, aggiungerlo lato app.
...
### **3. LED_STATE Notify con bladeState (gia' presente)**
...
```json
{
  ...
  "speed": 100,
  "enabled": true,
  "bladeState": "on",  // "off" | "igniting" | "on" | "retracting"
  "statusLedEnabled": true,
  "statusLedBrightness": 32,
  "foldPoint": 72,
  "gestureClashEffect": "clash",
...
Nota: il firmware include anche `autoIgnitionOnBoot`, `autoIgnitionDelayMs` e `motionEnabled`.
...
| **Brightness Slider** | `LED_BRIGHTNESS` | `{"brightness":80,"enabled":true}` | 0-255 (app puo' clampare se desiderato) |
...
Range firmware: 0-255 (app puo' applicare un cap locale se desiderato).
...
### **Implementazione firmware (gia' presente)**

#### 1. `DEVICE_CONTROL` Characteristic (estratto reale)
...
BLECharacteristic* pDeviceControlCharacteristic = pService->createCharacteristic(
...
class DeviceControlCallbacks : public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic* pCharacteristic) {
        ...
        if (command == "ignition") {
            ...
            controller->effectEngine->powerOff(false);
        } else if (command == "reboot") {
            ESP.restart();
        } else if (command == "sleep") {
            controller->effectEngine->powerOff(true);
        } else if (command == "boot_config") {
            // autoIgnitionOnBoot, autoIgnitionDelayMs, motionEnabled
        }
    }
};
```
...
#### 2. `EFFECTS_LIST` Characteristic (estratto reale)
...
#### 3. `bladeState` nel LED_STATE Notify (estratto reale)
...
Nota: la logica reale usa `LedEffectEngine::Mode` e `powerOn()/powerOff()`; vedi `src/BLELedController.cpp`.
...
### **Riassunto (gia' implementato nel firmware)**

1. ✅ **`DEVICE_CONTROL` characteristic** con comandi `ignition`, `retract`, `reboot`, `sleep`, `boot_config`
2. ✅ **`EFFECTS_LIST` characteristic** (READ) con lista effetti in JSON
3. ✅ **Campo `bladeState`** nel notify `LED_STATE`
...
1. Testare lettura/scrittura BLE con `ledsaber_control.py`
2. Allineare UI Flutter alla lista effetti dinamica
3. Validare OTA (MTU, write without response, rate limiting)

---

**Fine Specifica BLE API v3.1**
